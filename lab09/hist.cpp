...
// pdf may not be supported in certain gnuplot builds
// generate ps instead and then convert to pdf
void h1plot(h1 *hist, const char* filename){
  ...
  if (!outp) return;
  if (strlen(filename)) {
    fprintf(outp,"set term postscript color\n");
    fprintf(outp,"set output \"_%s_\"\n",filename);
  }
  ...
  if (strlen(filename)) 
    fprintf(outp,"!ps2pdf _%s_ %s; rm -f _%s_\n",filename,filename,filename);
  fclose(outp);
